...
int main(int argc, char **argv) {
  ...
  */
  ...
  printf("x: %p, x->data: %p, x->map: %p\n", (void *)x, (void *)x->data,
         (void *)x->map);
  printf("y: %p, y->data: %p, y->map: %p\n", (void *)y, (void *)y->data,
         (void *)y->map);
  printf("w: %p, w->data: %p, w->map: %p\n", (void *)w, (void *)w->data,
         (void *)w->map);
  printf("z: %p\n", (void *)z);
  if (z) {
    for (int i = 0; i < 3; i++) {
      printf("z[%d]: %p, z[%d]->data: %p, z[%d]->map: %p\n", i, (void *)z[i], i,
             (void *)z[i]->data, i, (void *)z[i]->map);
    }
  }
  ...
  */
  ...
  PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                        ...
                        maxind));
  // for now using macro petsc printf0 with no extra args
  PetscCall(
      PetscPrintf0(PETSC_COMM_WORLD, "All other values should be near zero\n"));
